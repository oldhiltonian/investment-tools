...
class Plots:    
    ...
    def plot_metrics(self):
        '''
        The plot_metrics method generates plots of financial metrics for a given company. The method first generates the necessary subplots to hold each metric type. Each metric type is then plotted on an individual axis within the metric type's subplot. Each metric's trend data is gathered, and its trend is plotted on the selected axis. A linear trendline is also plotted along with its corresponding R2 value. Finally, each metric type's subplot is given a title and added to the list of plots.
